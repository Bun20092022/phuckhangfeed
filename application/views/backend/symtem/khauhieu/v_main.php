	...
	<div class="card-body">
		...
		<form action="" method="post" id="form2">
			<div class="col-md-12 mb-3">
				...
				<input type="text" name="link_img" class="form-control" value="<?= $view_symtem_template['link_img']; ?>" id="form2">
			</div>
			<div class="col-md-12 mb-3">
				<label class="form-label">Tên button hiển thị</label>
				<select class="select2 form-control mb-3 custom-select" name="text_extend" style="width: 100%; height:36px;" id="form2">
						<optgroup label="Danh mục chọn">
							<!-- Lấy 5 cấp của danh mục cha -->
							<?php $list_lever1 = $this->Model_backend->get_all_asc('qh_setup_extend'); ?>
							<?php foreach ($list_lever1 as $value1): ?>
								<option value="<?= $value1['id']; ?>" <?php if($view_symtem_template['text_extend'] == $value1['id']){ echo 'selected'; } ?>><b><?= $value1['name']; ?></b></option>
							<?php endforeach ?>
						</optgroup>
					</select>
			</div>
			<div class="col-md-12">                                   
				<button class="btn btn-primary" type="submit" name="img" id="form2">Thêm link liên kết</button>
			</div><!-- end col -->
		</form>
	</div>
</div>
