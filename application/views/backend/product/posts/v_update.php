...
<script type="text/javascript">
   $(document).ready(function() {
         $.post('backend/product/posts/list_img', {"id_posts": <?= $id_posts; ?>}, function(data) {
            $('#loadajax').html(data);
         });
   });
   function deletex(id_img,id_posts) {
        $.post("backend/product/posts/delete_img", {
            id_img: id_img,
            id_posts: id_posts,
        }, function(data) {
            $("#loadajax").html(data);
         ...
         <div class="card-body">
            <h6 class="card-title">Nội dung chuyên mục</h6>
               ...
               <div class="mb-3">
                  ...
                  <input type="text" class="form-control" name="name_<?= $this->id_language;?>" id="name" onkeyup="ChangeToSlug();" oninput="checkname()" required  value="<?php if(strlen($name->{$this->id_language}) > 5){ echo $name->{$this->id_language}; }else{ echo $name->{'vn'}; } ?>" id="form1">
               </div>
               <div class="mb-3">
                  <label class="form-label">Url hiển thị</label>
                  <input type="text" class="form-control" <?php if(strlen($view['url_'.$this->id_language]) == 0){ echo 'id="slug"'; } ?> name="url_<?= $this->id_language;?>" value="<?= $view['url_'.$this->id_language];?>" id="form1">
               </div>
               <div class="mb-3">
                  <label class="form-label">Nội dung</label>
                  <textarea id="textarea" class="form-control" name="content_<?= $this->id_language;?>" id="form1"><?= $content->{$this->id_language}; ?></textarea>
                  <script>
               ...
               <div class="col-md-12 mb-3">
                 ...
                 <select class="select2 form-control mb-3 custom-select" name="post_category_id_ze" style="width: 100%; height:36px;" id="form1">
  ...
  <div class="tab-content">
    <div class="tab-pane p-3 active" id="home" role="tabpanel">
     <div class="mb-3">
      ...
      <input type="text" class="form-control" name="title_<?= $this->id_language;?>" required value="<?php if(strlen($title->{$this->id_language}) > 5){ echo $title->{$this->id_language}; }else{ echo $title->{'vn'}; } ?>" <?php if(strlen($title->{$this->id_language}) == 0){ echo 'id="title"'; } ?> id="form1">
   </div>
   <div class="mb-3">
      <label class="form-label">Mô tả ngắn</label>
      <textarea id="textarea" class="form-control" maxlength="225" rows="5" name="description_<?= $this->id_language;?>" placeholder="Giới hạn tốt nhất cho Seo là tối đa 225 ki tự hiển thị." id="form1"><?php if(strlen($description->{$this->id_language}) > 5){ echo $description->{$this->id_language}; }else{ echo $description->{'vn'}; } ?></textarea>
   </div>
   <div class="mb-3">
      <label class="form-label">Từ khóa</label>
      <input type="text" class="form-control" name="keywords_<?= $this->id_language;?>" value="<?php if(strlen($keywords->{$this->id_language}) > 5){ echo $keywords->{$this->id_language}; }else{ echo $keywords->{'vn'}; } ?>" id="form1">
   </div>
</div>
</div>        
</div><!--end card-body-->
...
<div class="col-md-4 grid-margin stretch-card">
   <div class="card">
      <div class="card-body"> 
         <button class="btn btn-sm btn-primary" type="submit" name="edit" id="form1">Chỉnh sửa thông tin</button>
   ...
   <div class="card-body"> 
      <div class="row">
         <div class="col-md-6">
            ...
            <select class="select2 form-control mb-3 custom-select" name="post_status" id="form1">
         ...
         <div class="col-md-6">
          ...
          <div class="input-group">                                            
            <input type="date" class="form-control" name="post_date" value="<?= date('Y-m-d',$view['post_date']); ?>" id="form1">
         </div>
      </div>
   </div>
</div>
...
<div class="card-body"> 
   <div class="row">
      <div class="col-md-12 mb-3">
        ...
        <select class="select2 form-control mb-3 custom-select" multiple="multiple" name="post_tags_id[]" style="width: 100%; height:36px;" id="form1">
   ...
   <div class="col-md-12 mb-3">
     ...
     <select class="select2 form-control mb-3 custom-select" name="post_templates_id" style="width: 100%; height:36px;" id="form1">
...
<div class="mb-3">
   ...
   <div class="row">
      <div class="col-md-8">
         <input id="xFilePath1" name="imgwebsite_<?= $this->id_language;?>" type="text" size="60" class="form-control" value="<?php if(strlen($imgwebsite->{$this->id_language}) > 5){ echo $imgwebsite->{$this->id_language}; }else{ echo $imgwebsite->{'vn'}; } ?>" id="form1">
      </div>
      <div class="col-md-4">
         <input type="button" class="btn btn-primary" value="Load ảnh" onclick="BrowseServer1();" id="form1">
      </div>
   </div>
</div>
<div class="mb-3">
   ...
   <div class="row">
      <div class="col-md-8">
         <input id="xFilePath" name="imgsocial_<?= $this->id_language;?>" type="text" size="60" class="form-control" value="<?php if(strlen($imgsocial->{$this->id_language}) > 5){ echo $imgsocial->{$this->id_language}; }else{ echo $imgsocial->{'vn'}; } ?>" id="form1">
      </div>
      <div class="col-md-4">
         <input type="button" class="btn btn-primary" value="Load ảnh" onclick="BrowseServer();" id="form1">
      </div>
   </div>
</div>
<div class="form-check form-switch">
   <label class="form-label">Cho phép bình luận</label>
   <input class="form-check-input" type="checkbox" name="post_comment" <?php if($view['post_comment'] == 2){ echo 'checked'; } ?> id="form1 form2">
</div> 
</div>
</div>
</div>
...
<form action="" method="get" id="form2">
   <input type="hidden" name="id_posts" value="<?= $id_posts ?>" id="form2">
<div class="card">
  <div class="card-header">
...
<div class="mb-3">
   ...
   <div class="row">
      <div class="col-md-8">
         <input id="xFilePath5" name="img_product" type="text" class="form-control" id="form2">
      </div>
      <div class="col-md-4">
         <input type="button" class="btn btn-primary" value="Load ảnh" onclick="BrowseServer5();">
      </div>
   </div>
</div>
<button class="btn btn-sm btn-primary" type="submit" name="add_img" id="form2">Thêm ảnh</button>
</div>
...
<script type="text/javascript">
   ...
   { 
      ...
      var submit = $("button[name='add_img']");
      
      submit.click(function()
      {
         var img_product = $("#xFilePath5").val(); //lấy giá trị input mật khẩu
        //kiểm tra xem bạn đã nhập email chưa
        if(img_product == ''){
            alert('Vui lòng link ảnh!');
            return false;
        }
        $('.loading').show();
        //Lấy toàn bộ dữ liệu trong Form
        var data = $('form#form2').serialize();
        $.ajax({
              type : 'get', //Sử dụng kiểu gửi dữ liệu POST
              url : 'backend/product/posts/add_img', //gửi dữ liệu sang trang loadaddsanpham với table được sử dụng
