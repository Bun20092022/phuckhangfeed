...
$view_category = $this->Model_frontent->view_id('qh_post_category',$view_url_post['post_category_id_ze'])
?>
<?php 
  // xem có thư mục con không nếu có thì lấy thư mục con ra

  $check_status = array(
      'father_id' =>$view_category['father_id'],
      'post_status' => 2,
    );
  ...
  <div class="container">
    <h2 style="margin-top: 260px;font-size: 40px;font-weight: bold;">
      ...
      <?php if(isset($info)){ ?>
            ...
            <a href="<?= base_url(); ?>backend/video/category/update/<?= $view_url_category['id']; ?>" target="_blank" style="font-size: 13px;"><i class="far fa-edit"></i></a>
         <?php } ?>
    </h2>
...
<?php 
        $id_posts_video = $view_url_post['id_posts_video'];
?>
  ...
  <div class="row" style="margin-top: 50px;margin-bottom: 100px;">
    <div class="col-md-12 text-center" style="margin-bottom: 50px;">
      <?php foreach ($list_category as $value): ?>
        <?php $name_category_video = json_decode($value['name']); ?>
        <a href="<?= base_url().$this->id_language; ?>/<?= $value['url_'.$this->id_language] ?>"><button style="background: #0C4DA2;color: #fff;border: none;padding: 6px 20px"><?= $name_category_video->{$this->id_language}; ?></button></a>
      <?php endforeach ?>
    </div>
    <div class="col-md-12">
      <div class="sidebartitle"><?= $name_post->{$this->id_language}; ?>
        <?php if(isset($info)){ ?>
            <a href="<?= base_url(); ?>backend/video/posts/update/<?= $view_url_post['id']; ?>" target="_blank" style="font-size: 13px;"><i class="far fa-edit"></i></a>
         <?php } ?>
      </div>
    </div>
    ...
    <div class="col-md-3">
      <div class="scrollbar" id="style-2">
        <div class="force-overflow">
          <div style="color: #000;font-weight: bold;font-size:20px;"><?= $name_category->{$this->id_language}; ?></div>
          <hr>
          <?php foreach ($list_video as $value): ?>
            <?php $father_id = json_decode($value['post_category_id']); ?>
            <?php if(in_array($view_url_category['id'], $father_id)){ ?>
